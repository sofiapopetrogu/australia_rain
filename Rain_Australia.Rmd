...
```
```{r LDA histogram}
# Here we plot stacked histograms representing the linear combination of variable that best represent the samples with their discriminant scores, split by each assigned class. The y-axis is the frequency of examples falling into each range of discriminant scores. From these plots, we observed little to no overlap across the two classes, which is an indicator of good separation between the two classes. 
...
cm_knn <- confusionMatrix(data =knn_pred_min , reference = rain_subset_test$RainTomorrow, mode ='everything')
...
ggplot(data = cm_table_knn, aes(x = Reference, y = Prediction, fill = Freq)) +
    ...
    geom_text(aes(label = Freq), color = "black", size = 8) +
    scale_fill_gradient(low = "white", high = "steelblue") +
    labs(title = paste("Confusion Matrix for KNN with F1-Score:", round(f1_score_knn, 3), "Error:", round(error_knn, 3) , "Accuracy:", round(accuracy_knn, 3)), x = "Target", y = "Prediction") +
    theme_minimal() +
    theme(axis.text = element_text(size = 8),
          plot.title = element_text(size = 8, face = "bold"))
...
# LDA Model
best_lda_model <- lda(data = rain_subset_train, RainTomorrow ~.,family = "binomial")

best_lda_predict <- predict(lda, rain_subset_test, type = "response")
best_post_lda<- best_lda_predict$posterior

# QDA Model
best_qda_model <- qda(data = rain_subset_train,RainTomorrow ~.,family = "binomial")

best_qda_predict <- predict(qda, rain_subset_test, type = "response")
...
prediction <- tibble(truth = as.factor(rain_subset_test$RainTomorrow))
prediction <- prediction %>% mutate(pred = as.numeric(best_glm_predict))%>%mutate(model= "GLM")%>%
  add_row(truth = as.factor(rain_subset_test$RainTomorrow), pred = as.numeric(best_post_lda[,2]),model= "LDA")%>%
  add_row(truth = as.factor(rain_subset_test$RainTomorrow), pred = as.numeric(best_post_qda[,2]),model= "QDA")%>%
  add_row(truth = as.factor(rain_subset_test$RainTomorrow), pred = as.numeric(best_ridge_predict),model= "Ridge")%>%
  add_row(truth = as.factor(rain_subset_test$RainTomorrow), pred = as.numeric(best_lasso_predict),model= "Lasso")

roc <- prediction %>% group_by(model) %>%
  roc(truth, pred, levels=c(0,1)) %/%
  plot(roc)


roc.out <- roc(prediction$truth, prediction$pred, levels=c(0,1))
...
roc_data <- prediction %>%
  group_by(model) %>%
  do(data.frame(roc(.$truth, .$pred, event.levels = "second"), model = unique(.$model)))
...
roc_score = roc(truth, pred)

roc <- prediction %>% group_by(model) %>%roc_curve(truth, pred, event_level = "second") %>%autoplot()roc54

# Calculate ROC curves for each model
roc_list <- lapply(split(prediction, prediction$model), function(df) {
...
model_colors <- rainbow(length(roc_list))
...
ggplot() +
  ...
  lapply(seq_along(roc_list), function(i) {
      labs(title = "ROC Curves", x = "False Positive Rate", y = "True Positive Rate", names(roc_list[[i]]))   
  }) +
